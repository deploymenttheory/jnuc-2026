...
Current pages: `s-workspace-round3`, `s07-checklist` (awaiting a decision). Decided: `s01-environment-band` (2026-08-27, option B - the Sandbox /
...
applied to the deck's `#s01` rules; the page is deleted, the index keeps the record as a
non-linked `.done` entry).
